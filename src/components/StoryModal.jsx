...
const StoryModal = ({ setShowModal, fetchStories }) => {
  ...
  const [backgroundColor, setBackgroundColor] = useState(bgColor[0])
  ...
  return (
    <div className="fixed inset-0 z-110 min-h-screen bg-black/80 backdrop-blur text-white flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="rounded-lg h-96 flex items-center justify-center relative" style={{ backgroundColor: backgroundColor }}>
          {mode === "text" && (
        ...
        <div className="flex mt-4 gap-2">
          {bgColor.map((color) => (
            <button key={color} className='w-6 h-6 rounded-full ring cursor-pointer' style={{ backgroundColor: color }} onClick={() => { setBackgroundColor(color) }} />
          ))}
        </div>
